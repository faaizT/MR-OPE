...
from obp_utils.helper_functions import (
    concatenate_feedback_data,
    load_feedback_data,
    save_feedback_data,
)
...
def set_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=100000,
        help="Number of training points for model",
    )
    parser.add_argument(
    ...
    parser.add_argument("--seed", type=int, default=1, help="Seed for the run")
    parser.add_argument(
        "--beta_behav",
        type=float,
        default=1,
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default="True",
        help="Use generated data in data_dir. If false, generate and save new data",
    )
    parser.add_argument(
        "--data_dir",
        type=str,
        default="/data/ziz/not-backed-up/taufiq/continuous_reward_sine_ax_alpha_target_policy_large_action_spaces",
        help="Directory of saved training data",
    )
    parser.add_argument("--n_actions", type=int, default=1000, help="Number of actions")
    parser.add_argument(
        "--dim_context", type=int, default=100, help="Context dimension"
    )
    parser.add_argument(
        "--reward_std",
        type=float,
...
def main(args):
    ...
    data_dir = f"{args.data_dir}/n_actions{args.n_actions}-dim_context{args.dim_context}-n_train{n_train}"
    if args.use_generated_data:
        bandit_feedback_train = load_feedback_data(data_dir)
    else:
        n_reps = 500
        bandit_feedback_train = dataset.obtain_batch_bandit_feedback(
            n_rounds=n_train // n_reps
        )
        for i in tqdm(range(n_reps - 1)):
            bandit_feedback_train_rep = dataset.obtain_batch_bandit_feedback(
                n_rounds=n_train // n_reps
            )
            bandit_feedback_train = concatenate_feedback_data(
                bandit_feedback_train, bandit_feedback_train_rep
            )
        logging.info("Data generated")
        logging.info("Saving generated training data")
        Path(data_dir).mkdir(parents=True, exist_ok=True)
        save_feedback_data(bandit_feedback_train, dir=data_dir)
        logging.info("Generate training data saved for later")
    ...
    ope = create_ope_object(
        ...
        pi_b_scores=pi_b_scores_train[n_train // 2 :],
        pi_e_scores=pi_e_scores_train[n_train // 2 :],
        reward=bandit_feedback_train["reward"][n_train // 2 :],
        reward_type=reward_type,
    ...
    tag = create_tag(args, ignore_cols=["results_dir", "save_ratios", "data_dir"])
